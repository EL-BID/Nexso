...
function activateUploader(SolutionId) {
    ...
    dropbox.filedrop({
        ...
        maxfiles: 1,
        maxfilesize: 2,
        url: window.location.protocol+"//"+window.location.host+'/DesktopModules/NexsoServicesV2/API/solution/UploadBanner',

        uploadFinished: function (i, file, response) {

            if (response.Filename.length>0) {
                deactivateUploader(SolutionId);
...
function deactivateUploader(SolutionId) {
    $('#dropbox').hide();
    $('#dropbox').off();
}
...
function acivateUploaderButton(SolutionId) {
    SolutionIdVar = SolutionId;
...
function uploadFiles(event) {
    ...
    formData.append('SolutionId', SolutionIdVar);
    ...
    $.ajax({
        ...
        success: function (response, textStatus, jqXHR) {
            if (response.Filename.length>0) {
                deactivateUploader(SolutionIdVar);
                stepController('Repositioning', response.Filename + response.Extension);
...
function RepositioningImage(SolutionFile) {

    $(".picturecontainer").css({
        height: $(".headerimage").width() / 2, border: "1px solid #888", overflow: "hidden", cursor: "-webkit-grab"
    });
    $(".headerimage").attr("src", '/Portals/0/ModIma/TempImages/' + SolutionFile + '?timespan=0').load(
        function () {
...
function cropImage(arg) {
    if (globalPositionY != null) {
        $.ajax({
            ...
            url: window.location.protocol+"//"+window.location.host+"/DesktopModules/NexsoServicesV2/API/solution/CropSaveBanner?solutionId=" + SolutionIdVar,
            ...
            success: function (data, response, i) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i].Filename.toLowerCase().indexOf("cropbig") >= 0) {
                        ...
                        originalFileName = '/Portals/0/ModIma/HeaderImages/' + data[i].Filename + data[i].Extension+'?timespan=0';
