CREATE PROCEDURE [dbo].[dnn_EasyDNNNewsGetItemsPerCategory]
	@PortalID int,
	@UserID int,
	@MenuModuleID int,
	@DefaultTabID int,
...
BEGIN
	...
	(
		SELECT Cat.* FROM dbo.[dnn_EasyDNNNewsCategoryList] AS Cat INNER JOIN @UserViewCategoriesWithFilter AS uvc ON uvc.CategoryID = Cat.CategoryID WHERE Cat.PortalID = @PortalID AND Cat.CategoryID NOT IN (SELECT lctemp.CategoryID FROM LocCategories as lctemp)
	)
	 ...
	 FROM dbo.[dnn_EasyDNNNews] as n
	  ...
	  INNER JOIN @tempMenuCats AS cidl ON cidl.CategoryID = c.CategoryID AND cidl.Show = 1
	   WHERE n.ArticleID IN(
		...
		INNER JOIN  dbo.[dnn_EasyDNNNewsCategories]  as cat ON na.ArticleID = cat.ArticleID
		INNER JOIN @tempMenuCats AS cidl ON cidl.CategoryID = cat.CategoryID AND cidl.Show = 1
		INNER JOIN dbo.[dnn_EasyDNNNewsContentLocalization] AS ncl ON ncl.ArticleID = na.ArticleID AND ncl.LocaleCode = @LocaleCode
		   WHERE na.PortalID=@PortalID
		   AND ((@EditPermission = 1) OR (
				(@UserCanApprove = 1 OR na.Approved = 1)
				 AND (na.Active = 1 OR na.UserID=@UserID)
