CREATE PROCEDURE [dbo].[dnn_GetUsersBasicSearch]
(
	@PortalID int,					-- portal
...
AS
	...
	SELECT @pivotSql = dbo.dnn_GetProfileFieldSql(@PortalID, '')

	-- Setup FieldName Field List for temporary table
	DECLARE @fieldNames nvarchar(max)
	SELECT @fieldNames = dbo.dnn_GetProfileFieldSql(@PortalID, ' nvarchar(max)')
