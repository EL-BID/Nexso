CREATE PROCEDURE [dbo].[dnn_GetUsersAdvancedSearch] 
(
    @PortalID int,                          -- portal                
...
AS
    ...
    DECLARE @pivotSql nvarchar(max) SELECT @pivotSql = dbo.dnn_GetProfileFieldSql(@PortalID, '')

    -- Get User specific columns
    DECLARE @UserColumns TABLE(ColumnName NVARCHAR(100))
    INSERT INTO @UserColumns SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'dnn_vw_Users'
    ...
    INSERT INTO @PropertyNamesTable SELECT * FROM dbo.dnn_ConvertListToTable(',', @PropertyNames)                      
    INSERT INTO @PropertyValuesTable SELECT * FROM dbo.dnn_ConvertListToTable(',', @PropertyValues)
    ...
    INSERT INTO @UserFiltersTable SELECT * FROM dbo.dnn_ConvertListToTable(',',@PropertyNames) WHERE RowValue IN (SELECT * FROM @UserColumns)
    ...
    IF ((SELECT COUNT(*) FROM @PropertyNamesTable) > 0) AND ((SELECT COUNT(*) FROM @PropertyValuesTable)> 0) 
	    BEGIN
            ...
            IF @userFilterCount > 0 BEGIN SET @userFilterJoin = ' INNER JOIN dnn_vw_Users u ON u.UserId = p.UserId ' END

            -- Determining the Group By Clause -- dependant on types of filters used
            SELECT @profilePropertyCount = COUNT(*) FROM dbo.dnn_ConvertListToTable(',', @PropertyNames)
            WHERE RowValue IN (SELECT PropertyName FROM dnn_ProfilePropertyDefinition WHERE PortalID = @PortalId)
            ...
            SET @filterSql = ' DECLARE @MatchingUsers TABLE (UserID INT, Occurrances INT) INSERT INTO @MatchingUsers SELECT p.UserID, COUNT(*) AS occurances ' 
                                        + ' FROM dbo.dnn_vw_profile p ' + @userFilterJoin
        ...
        IF @FilterRoleId <> -1 
            BEGIN
                ...
                SET @roleFilter = ' JOIN dnn_UserRoles UR ON U.UserID = UR.UserID AND UR.RoleID = ' + CONVERT(nvarchar(20), @FilterRoleId)
            END
        ...
        IF @RelationshipTypeId <> -1  
            BEGIN
                SET @roleAndRelationshipFlag = 1
                SET @relationshipFilter = ' JOIN dnn_Relationships REL ON REL.PortalID = ' + CONVERT(nvarchar(20), @PortalID)
                                            + ' AND RelationshipTypeID = ' + CONVERT(nvarchar(20), @RelationshipTypeId) 
                                            + ' JOIN dnn_UserRelationships UREL ON REL.RelationshipID = UREL.RelationshipID AND
        ...
        SET @roleAndRelationshipFilter =  ', RoleAndRelationUsers AS ( SELECT U.userId FROM dnn_vw_Users U ' + @roleFilter + @relationshipFilter + ' )' 
        ...
        IF (@SortBy IS NOT NULL AND @SortBy <> '') AND @sortByUserProperty <> 1
	        BEGIN -- Sort By Profile Property
                SET @sortSql = dbo.dnn_GetSortSql(@SortBy,@SortAscending,'UserID')
                SET @propertySort = dbo.dnn_GetSortSql('PropertyValue',@SortAscending,'UserID')
                SET @filterSortSql = ' ;WITH SortedUsers AS ( SELECT ROW_NUMBER() OVER( ' + @propertySort + ' ) AS RowNumber, *  ' 
                                                + ' FROM dnn_vw_Profile WHERE PortalId = ' + CONVERT(nvarchar(20), @PortalID) + ' AND PropertyName = ''' + @SortBy + ''' )'
        ...
        ELSE
		    BEGIN   
                ...
                IF @sortByUserProperty = 1 BEGIN SET @sortSql = dbo.dnn_GetSortSql(@SortBy,@SortAscending,'UserID')END
                                
                -- Default: Sort By UserID
                ELSE BEGIN SET @sortSql = dbo.dnn_GetSortSql('UserID',@SortAscending,'UserID') END                        
                SET @filterSortSql = ' ;WITH SortedUsers AS ( SELECT ROW_NUMBER() OVER( ' + @sortSql + ' ) AS RowNumber, * '
                                                + ' FROM dnn_vw_Users WHERE (PortalID = ' + CONVERT(nvarchar(20), @PortalID) + ' OR PortalID Is NULL) AND IsDeleted = 0)'
                                                + ' , MatchingSorted AS ( SELECT ROW_NUMBER() OVER(ORDER BY [RowNumber]) AS RowNumber, s.UserId FROM SortedUsers s '
                                                + @filterJoin + ' ) '
	        END
        ...
        SET @sql = @filterSql
                ...
                + ' ;WITH PivotedUsers AS ( SELECT U.UserID, U.PortalID, U.Username, U.Email, U.DisplayName, U.IsSuperUser, U.IsDeleted, U.CreatedOnDate,        
                                            ...
                                            FROM   dnn_vw_Users AS U
                                                INNER JOIN dnn_UserProfile AS P ON U.UserID = P.UserID
                                                LEFT OUTER JOIN dnn_ProfilePropertyDefinition AS Prop ON 
